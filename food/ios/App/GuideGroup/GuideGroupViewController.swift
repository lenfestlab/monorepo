...
//    context.analytics.log(.tapsOnGuideCell(category: category))
    let guidesController = GuidesViewController(context: context)
    guidesController.title = guideGroup.title
//    guidesController.topBarIsHidden = true
    self.navigationController?.pushViewController(guidesController, animated: true)
  }
